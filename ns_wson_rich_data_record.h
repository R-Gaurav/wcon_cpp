...
//////////////////////////////////////////////////////////////////////////
//
// ns_wson_rich_data_record.h
// 
// This class ns_wson_rich_data_record_element implements the parser/writer for the full
// Worm tracker Commons Object Notation schema
// wcon_schema.json 
// as in specified August 2018.
//
// Currently, custom variables are not implemented, and worm images are
// left as MIME-endcoded strings
//
// Nicholas Stroustrup 
...
struct ns_wson_pixel_walk_record {
	...
	bool operator==(const ns_wson_pixel_walk_record & r) const {
...
public:
	...
	bool operator==(const ns_wson_rich_data_record_element & r) const {
		...
		bool b = ns_quick_type_conversion::compare_double(t, r.t) &&
	...
	static int number_of_additional_json_fields() { return 7; }
...
std::string ns_wson_rich_data_record_element::to_string(const ns_wson_rich_data_record_element::ns_head & head) {
	switch (head) {
	case L: return "L";
	case R: return "R";
	case head_NA: return "?";
...
std::string ns_wson_rich_data_record_element::to_string(const ns_wson_rich_data_record_element::ns_ventral & ventral) {
	switch (ventral) {
	case CW: return "CW";
	case CCW: return "CCW";
...
ns_wson_rich_data_record_element::ns_head ns_wson_rich_data_record_element::head_from_string(const std::string & str) {
	if (str == "L") return L;
	if (str == "R") return R;
	if (str == "?") return head_NA;
	throw std::exception((std::string("Unknown head spec ") + str).c_str());
}
ns_wson_rich_data_record_element::ns_ventral ns_wson_rich_data_record_element::ventral_from_string(const std::string & str) {
	if (str == "CW") return CW;
	if (str == "CCW") return CCW;
	if (str == "?") return ventral_NA;
	throw std::exception((std::string("Unknown ventral spec ") + str).c_str());
}
nlohmann::json ns_wson_rich_data_record_element::to_json() const {
	nlohmann::json j;
	j["id"] = id;
	j["t"] = t;
	j["x"] = x;
...
nlohmann::json ns_wson_rich_data_record_element::add_subclass(int i) const {
	nlohmann::json j;
	if (i == 6) {
		if (walk.is_specified())
			j = walk.to_json();
...
template<class T>
bool ns_wson_rich_data_record_element::set_value(const std::string & key, const T & val, const std::string & subclass_name) {
	if (subclass_name == "") {
		std::string tmp;
		if (key == "px") {
			if (json_reader_current_px_index > 10)
				std::cerr << "WHA";
			...
			if (json_reader_current_py_index > 10)
				std::cerr << "WHA";
...
std::string ns_wson_rich_data_record_element::additional_json_field_name(const int &i) {
	switch (i) {
	case 0: return "cx";
	case 1: return "cy";
	case 2: return "px";
	...
	case 6: return ns_wson_pixel_walk_record::json_name();
	default:
		throw std::exception("Unknown field id");
	}
}

bool ns_wson_rich_data_record_element::json_field_is_a_known_subclass(const std::string & key) {
	return key == ns_wson_pixel_walk_record::json_name();
}
ns_wson_data_element_member_type ns_wson_rich_data_record_element::additional_json_field_type(const int &i) {
	switch (i) {
	case 0:
	case 1:
		return ns_double;
	...
	case 5:
		return ns_string;
	case 6: return ns_subclass;
	default: return ns_unknown;
	}
}

double ns_wson_rich_data_record_element::get_additional_json_field_value_double(const int &i) const {
	switch (i) {
	case 0: return cx;
	case 1: return cy;
	default:
...
std::string ns_wson_rich_data_record_element::get_additional_json_field_value_string(const int &i) const {
	char buf[256];
	switch (i) {
...
const std::vector<double> * ns_wson_rich_data_record_element::get_additional_json_field_value_vector_double(const int &i) const {
	switch (i) {
	case 2: return &px;
	case 3: return &py;
	default:
...
const std::vector < std::string > * ns_wson_rich_data_record_element::get_additional_json_field_value_vector_string(const int &i) const {
	throw std::exception("No string vectors supported");
}

nlohmann::json ns_wson_pixel_walk_record::to_json() const {
	nlohmann::json j;
	j["4"] = four;
	j["n"] = n;
	j["px"] = px;
